...
class KidsOutfitCard extends HTMLElement {
  ...
  static getStubConfig(hass){return {entity:Object.keys(hass?.states||{}).find(id=>hass.states[id].attributes?.schema_version===1 && hass.states[id].attributes?.outfit) || 'sensor.kids_outfit'};}
  render(){
    ...
    const invalid=!state || ['unavailable','unknown'].includes(state.state) || !a || a.schema_version!==1 || !Number.isFinite(Date.parse(a.valid_until)) || Date.now()>Date.parse(a.valid_until);
    const renderKey=JSON.stringify([config,state,locale,this._hass.config?.unit_system,this._hass.config?.time_zone,invalid]);
    if(renderKey===this._renderKey)return;
    this._renderKey=renderKey;
    if(invalid){this.shadowRoot.innerHTML=`<style>${styles}</style><ha-card><div class="error" role="status"><div class="error-icon" aria-hidden="true">☁</div><h1>${escape(t('unavailable'))}</h1><p>${escape(t('unavailable_detail'))}</p></div></ha-card>`;return;}
    ...
    const items=[a.top,a.outer,a.bottom,a.shoes,...(a.accessories||[])].filter(Boolean);
    const list=(keys,cls='')=>`<ul class="items ${cls}">${keys.map(key=>`<li class="item" data-item="${escape(key)}">${icon(key)}<span>${escape(t(key))}</span></li>`).join('')}</ul>`;
    const notes=a.notes||[];
    const detailOpen=this.shadowRoot.querySelector('details')?.open;
...
class KidsOutfitEditor extends HTMLElement {
  ...
  render(){
    ...
    if(!this._form){this.shadowRoot.innerHTML='<ha-form></ha-form>';this._form=this.shadowRoot.firstElementChild;this._form.addEventListener('value-changed',e=>{this._config={...this._config,...e.detail.value};this.dispatchEvent(new CustomEvent('config-changed',{detail:{config:this._config},bubbles:true,composed:true}));});}
